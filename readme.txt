...
* Release snapshots no longer cache GitHub's full asset objects (only name, URL, and size); on Memcached-backed sites a download-heavy repository's snapshot exceeded the 1 MB item limit and was refetched on every check.
...
* The repository table's generate button is now labeled "Generate draft" and the version picker explains that manually generated posts are always drafts for review, clarifying that the Status setting applies to posts created by the scheduled check.
* Adding a repository now verifies it exists on GitHub, so a mistyped path is rejected with a helpful message instead of being silently tracked forever.
