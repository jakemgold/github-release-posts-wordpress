...
* Monorepos are now monitored per package — in both modes: with chosen packages, and in the default all-packages mode (detected automatically from the repository's release tags). When several packages release between scheduled checks (common for coordinated monorepo releases), every new release gets a post — previously only the single newest across the whole repository was processed and sibling releases could be skipped permanently. Version comparison understands package tags within a package's own release stream, and releases that already have a post (for example, manually generated drafts) are never re-processed by the scheduled check. The first check after upgrading records the current state without generating, so no burst of posts appears; backfilling stays a deliberate manual action. Repositories added going forward are baselined at add time, so a repository added before its first release still generates that first release as promised. A repository that later becomes a monorepo is detected within about a week, even when its newest release uses a plain repository-wide tag, and the version picker's "latest" marker now always matches the release automatic generation would choose. Manual generation honors the repository's "Include pre-releases" setting exactly like the version picker (pre-release-only projects can generate their releases when opted in, and explicit pre-release requests are refused when not), and repositories mixing package releases with plain repository-wide tags are monitored per stream consistently everywhere — while keeping the normal initial draft on add, since the package chooser applies to repositories with two or more recognized packages.
