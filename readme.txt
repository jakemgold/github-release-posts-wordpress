...
* Package-style naming for post titles, URL slugs, and admin labels engages automatically once the plugin observes that a repository releases multiple packages — a freshly added monorepo gets titles like "Astro cloudflare 14.1.4 — …" with no configuration (choosing packages also turns it on, covering uncommon tagging schemes supplied via the developer filter). Repositories that release a single package keep their existing titles, slugs, and labels byte-for-byte after upgrading — even when their tags look like package tags (e.g. `myplugin@1.2.3`). Monorepos tracked before this version pick up package naming for future posts after their next scheduled check; existing posts are never renamed.
* Deep research (when enabled) now builds a package release's commit history against that same package's previous release, instead of the repository's most recent release overall — so monorepo posts researched deeply no longer draw on commit ranges between unrelated packages.
...
* Adding a repository now verifies it exists on GitHub, so a mistyped path is rejected with a helpful message instead of being silently tracked forever. (Private repositories need a Personal Access Token with access to them configured first.)
