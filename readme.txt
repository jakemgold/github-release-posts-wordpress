...
* Monorepos are now monitored per package: when several selected packages release between scheduled checks (common for coordinated monorepo releases), every new release gets a post — previously only the single newest across the whole repository was processed and sibling releases could be skipped permanently. Version comparison also understands package tags, so a back-ported older version can no longer displace a newer one.
* Package-style display formatting for titles, slugs, and the Last Post column now applies to dash-style tags (like `my-plugin-v1.2.3`) only when the repository has packages configured — single-package repositories keep their existing titles and slugs exactly.
